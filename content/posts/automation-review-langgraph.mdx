...
<ReviewDashboard
  ...
  problems={[
    ...
    { icon: "stream", title: "Streaming and partial results" },
    { icon: "group", title: "Sub-agent composition" },
  ]}
  reach={{
    primary: "~62M",
  ...
  cons={[
    ...
    "Docs uneven across runtimes; Python deeper than JS",
    "LangChain peer-dependency gravity is real",
...
Public launch. Python v0.0.10 on 9 January, JS v0.0.1 on 18 January, formal announcement on 22 January. Both releases were spartan: `StateGraph`, a handful of node helpers, integration with the existing LangChain ecosystem for prompts and tools.

</TimelineEntry>

<TimelineEntry date="Mid 2024">

Stream support, conditional edges, the first usable checkpointing layer. LangGraph starts showing up in serious production case studies.

</TimelineEntry>

<TimelineEntry date="Late 2024 to early 2025">

Persistence backends (in-memory, Postgres, SQLite), the `Send` / `Map` primitives for fan-out, time-travel debugging, and LangGraph Studio. The Python side gets an adoption push from the LangChain v0.3 release that aligned the two ecosystems.

</TimelineEntry>

<TimelineEntry date="Late 2025">

The 1.0 line. v1.0 alpha in September; v1.0 GA on 22 October 2025. The API stabilises; the TypeScript types get markedly better. The commercial deployment product has been renamed twice: launched as **LangGraph Cloud** (June 2024), rebranded to **LangGraph Platform** (October 2024) when self-hosted and bring-your-own-cloud landed, then renamed to **LangSmith Deployment** (October 2025) as part of a broader product-naming cleanup.
...
| You need the production-concern surface (persistence, streaming, human approval, observability) provided by the framework rather than reinvented. | You will not tolerate the LangChain peer-dep gravity. |
...
**The LangChain peer-dependency gravity is real.** ("Peer dependencies" are packages a library expects you to install alongside it.) You can use LangGraph without the rest of LangChain, but the path of least resistance pulls you in. If you prefer thinner installs, plan that boundary deliberately.
...
A few things the poster cannot carry, but the verdict should.
